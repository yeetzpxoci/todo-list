...
function createProject(name) {
    ...
    localStorage.getItem("projects").push(newProject);
}

function changeCurrentProject(name) {
    for (let i = 0; i < localStorage.getItem("projects").length; i++) {
        if (localStorage.getItem("projects").name === name) {
            localStorage.getItem(localStorage.getItem("currentProject")) = projects[i];
        }
    }
}
...
localStorage.setItem("currentProject", localStorage.getItem("defaultProject"));
